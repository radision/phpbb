...
$lang = array_merge($lang, array(
	...
	'DEFAULT_STYLE'					=> '默认风格',
	'DEFAULT_STYLE_EXPLAIN'			=> '新用户默认风格。',
	'DISABLE_BOARD'					=> '关闭论坛',
	'DISABLE_BOARD_EXPLAIN'			=> '这将让用户无法使用论坛。 您可以输入一段简单的文字描述原因(255个字符以内)。',
	'DISPLAY_LAST_SUBJECT'			=> '显示最新添加到版面列表里的帖子标题',
...
// Board Features
$lang = array_merge($lang, array(
	'ACP_BOARD_FEATURES_EXPLAIN'	=> '这里您可以启用/禁用论坛的功能',
...
$lang = array_merge($lang, array(
	...
	'MAX_FILESIZE_EXPLAIN'			=> '针对上载的头像文件，如果设为 0 的话上传文件大小只受限于您的 PHP 配置。',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_BBCODE_PM'			=> '在私人短信中允许 BBCode',
	'ALLOW_FLASH_PM'			=> '允许使用 <code>[FLASH]</code>',
	'ALLOW_FLASH_PM_EXPLAIN'	=> '这个选项设置是否允许在私人短信中使用 flash，在这里启用后用户依然要视论坛的权限而决定是否可用。',
...
$lang = array_merge($lang, array(
	...
	'MIN_CHAR_LIMIT_EXPLAIN'		=> '用户在发帖或发私信时内容的最小字符数。最小值是 1。',
	'POSTING'						=> '发帖',
...
$lang = array_merge($lang, array(
	...
	'ACC_ACTIVATION_WARNING'		=> '注意，目前选择的用户帐号激活方法需要启用“允许论坛发送Email”，否则注册将被停用。建议您选择其它的帐号激活方法或者重新启用“允许论坛发送Email”。',
	'NEW_MEMBER_POST_LIMIT'			=> '新用户发帖数目',
	...
	'NEW_MEMBER_GROUP_DEFAULT_EXPLAIN'	=> '如果设置为是， 并且设定了【新用户发帖数目】限制，新用户不仅会被加入到 <em>新注册用户</em> 组，这个用户组也会成为他的默认组。这将方便管理员为该组指定一个默认等级和组头像。',
...
// Cookie Settings
$lang = array_merge($lang, array(
	'ACP_COOKIE_SETTINGS_EXPLAIN'		=> '以下设定了送往用户浏览器的数据。在大多数情况下使用默认设置就已足够。如果您自行调整，不正确的设置将会使用户无法登录论坛。如果你的用户无法保持登入状态，可以访问 <b><a href="https://www.phpbb.com/support/go/cookie-settings/">phpBB.com Knowledge Base - Fixing incorrect cookie settings</a></b> 。',
	'COOKIE_DOMAIN'				=> 'Cookie 作用域',
	'COOKIE_DOMAIN_EXPLAIN'		=> '大多数时候，Cookie作用域是可选项。如果你不确定，可留空。<br><br> 比如你的论坛同其它软件组合在一起，或者有多个域名，就需要决定 cookie域。 如果你的域名是  <i>example.com</i> 和 <i>forums.example.com</i> ，或者是  <i>forums.example.com</i> 和 <i>blog.example.com</i>。 去掉子域名，直到你找到共同域， <i>example.com</i> 。 在共同域名前加上一个英文句点  .example.com （注意看开头的句点）。',
	'COOKIE_NAME'				=> 'Cookie 名称',
	'COOKIE_NAME_EXPLAIN'		=> '这可以是任何你希望的，确保原创。当你修改cookie设置时，次名称也应当被修改。',
...
$lang = array_merge($lang, array(
	...
	'CONTACT_US_ENABLE_EXPLAIN'		=> '此页允许用户发送邮件给论坛管理员。请注意论坛全局的电邮选项必须同时启用才生效，你可以在 综合 》 客户端通信 》 Email 设定 设置该选项。 ',
...
$lang = array_merge($lang, array(
	...
	'LIMIT_LOAD'					=> '系统负载限制',
	'LIMIT_LOAD_EXPLAIN'			=> '如果系统一分钟内的平均负载超过这个值论坛将自动关闭。值 1.0 等于单颗处理器的100%使用率，这只工作于基于UNIX的系统，并且可以访问得到使用率信息。当论坛程序无法获得负载率的值的时候，会被重置为 0',
	'LIMIT_SESSIONS'				=> '会话数限制',
	'LIMIT_SESSIONS_EXPLAIN'		=> '如果一分钟内的会话数超过这个值论坛将自动关闭。设置为 0 将不作限制。',
	'LOAD_CPF_MEMBERLIST'			=> '允许风格在会员列表中显示自定义资料',
	'LOAD_CPF_PM'					=> '在私信里显示自定义资料',
...
$lang = array_merge($lang, array(
	...
	'AUTH_PROVIDER_OAUTH_EXPLAIN'				=> '每个 OAuth 提供商需要唯一的密码和密钥，为了验证外部服务器。这些应该被 OAuth 服务提供，当您注册您的网站时应该输入准确。<br>任何没有密钥和密码的服务在这儿将不可用，也要注意用户仍可以注册和登录使用 DB 验证插件。',
...
$lang = array_merge($lang, array(
	...
	'MOD_REWRITE_ENABLE_EXPLAIN' => '启用后包含 ’app.php’ 的路径将重写为移除文件名(例如app.php/foo 变成 /foo)。<strong>必须有 Apache服务器的mod_rewrite 组件才能正常工作；如果此选项启用但没有 mod_rewrite，路径将损坏。</strong>。',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_PASSWORD_RESET_EXPLAIN'	=> '确定是否能使用在登录页的"我忘记了自己的密码"链接来找回密码。如果使用外部验证机制您可能想禁用此功能。',
...
$lang = array_merge($lang, array(
	...
	'SMTP_VERIFY_PEER_NAME_EXPLAIN'	=> '要求验证 SMTP 服务器的节点名字，并使用 SSL/TLS 连接。 <br><em><strong>警告：</strong> 连接到未验证的节点名字可能会导致安全隐患。</em>',
 	'TEST_EMAIL_SENT'				=> '测试邮件已发送。<br />如果你没有收到，请检查你的 Email 设置。<br><br>访问 <a href="https://www.phpbb.com/community/">phpBB support forums</a> 获取帮助。',

	'TEST_EMAIL_SENT'				=> '测试 email 已发送。<br />如果您没有收到它，请检查您的 email 设置。<br /><br />请到 <a href="https://www.phpbb.com/community/">phpBB support forums</a> 寻求支持。',
	'USE_SMTP'						=> '使用 SMTP 服务器发送 email',
	'USE_SMTP_EXPLAIN'				=> '选择“是”。如果您向通过其它服务器而不是本地 mail函数发送 email。',
));

// Jabber settings
$lang = array_merge($lang, array(
	'ACP_JABBER_SETTINGS_EXPLAIN'	=> '这里您可以启用并控制用户使用 Jabber 发送及时消息和论坛通知。Jabber 是任何人都可以使用的开放协议。一些 Jabber 服务器提供允许您联系其他网络用户的通道。并非所有的服务器都提供这样的通道，协议上的变化将使得操作失败。请确认输入的是已经注册的帐号信息，phpBB 将会使用您这里输入的数据。',
