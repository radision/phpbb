...
$lang = array_merge($lang, array(
	...
	'ACP_GROUPS_PERMISSIONS_EXPLAIN'			=> '这里您可以将全局权限分配到组 - 用户权限、超级版主权限和管理员权限。用户权限包括的功能如使用头像、发送私信等等；超级版主权限如批准帖子、管理主题、管理封禁等等；管理员权限如变更权限、设定 BBCode、管理版面等等。只有在极少数的情况下才需要改变单个用户权限，最好的方法是将用户归组然后给用户组分配权限。',
	...
	'ACP_USERS_PERMISSIONS_EXPLAIN'				=> '这里您可以给用户分配全局权限 - 用户权限、超级版主权限和管理员权限。用户权限包括的功能如使用头像、发送私信等等；超级版主权限如批准帖子、管理主题、管理封禁等等；管理员权限如变更权限、定义 BBCode、版面管理等等。要变更大量用户的权限推荐使用组权限系统，用户权限应该极少被变更，优先的方法是将用户归组然后分配组权限。',
